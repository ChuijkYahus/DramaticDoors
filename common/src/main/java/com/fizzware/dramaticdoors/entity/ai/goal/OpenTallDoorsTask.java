...
public class OpenTallDoorsTask
{
    ...
    public static BehaviorControl<LivingEntity> create() {
        ...
        return BehaviorBuilder.create(context -> context.group(context.present(MemoryModuleType.PATH), context.registered(MemoryModuleType.DOORS_TO_CLOSE), context.registered(MemoryModuleType.NEAREST_LIVING_ENTITIES)).apply(context, (pathRaw, doorsToClose, mobs) -> (world, entity, time) -> {
            ...
            if (blockState.is(DDBlockTags.MOB_INTERACTABLE_TALL_DOORS, state -> state.getBlock() instanceof TallDoorBlock)) {
            ...
            if ((blockState2 = world.getBlockState(blockPos2 = pathNode2.asBlockPos())).is(DDBlockTags.MOB_INTERACTABLE_TALL_DOORS, state -> state.getBlock() instanceof TallDoorBlock) && !(doorBlock2 = (TallDoorBlock)blockState2.getBlock()).isOpen(blockState2)) {
